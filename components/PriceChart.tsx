...
export function PriceChart() {
  ...
  useEffect(() => {
    ...
    const symbolTrades = trades
      .filter(
        (trade) =>
          trade.symbol === selectedSymbol && trade.timestamp >= startTime
      )
          ...
          {chartData.length > 0 ? (
            ...
            <div className="h-full flex items-center justify-center text-muted-foreground">
              <div className="text-center">
                <div className="text-lg mb-2">No price data to show yet</div>
                <div className="text-sm">
                  Once trades start coming in, you'll see the price chart here.
                </div>
              </div>
            </div>
          )}
